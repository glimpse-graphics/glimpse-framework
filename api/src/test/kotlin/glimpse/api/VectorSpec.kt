	...
	init {
		...
		"Spherical coordinates" should {
			"translate to proper cartesian coordinates" {
				Vector(1f, Angle.NULL, Angle.NULL) shouldBeRoughly Vector(0f, 0f, 1f)
				Vector(2f, Angle.RIGHT, Angle.NULL) shouldBeRoughly Vector(2f, 0f, 0f)
				Vector(3f, Angle.RIGHT, Angle.RIGHT) shouldBeRoughly Vector(0f, 3f, 0f)
			}
		...
		"Cross products of two vectors" should {
			...
			"have a magnitude equal to product of the magnitudes of two orthogonal vectors" {
				forAll(floats, floats, angles, angles) { x, y, inclination, azimuth ->
					(Vector(x, inclination, azimuth) * Vector(y, inclination + Angle.RIGHT, azimuth)).magnitude isRoughly (x * y)
				}
			}
			"be anticommutative" {
		...
		"Dot products of two vectors" should {
			...
			"be 0 for orthogonal vectors" {
				forAll(floats, floats, angles, angles) { x, y, inclination, azimuth ->
					Vector(x, inclination, azimuth) dot Vector(y, inclination + Angle.RIGHT, azimuth) isRoughly 0f
				}
			}
			"be commutative" {
				forAll(vectors, vectors) { a, b ->
		...
		"Vectors" should {
			...
			"not be parallel if they differ by angle" {
				forAll(floats, floats, angles, angles) { x, y, inclination, azimuth ->
					!(Vector(x, inclination, azimuth) parallelTo Vector(y, inclination + Angle.RIGHT, azimuth))
				}
			}
		}
